...
vim.cmd([[
   function! ShowCwd(a,b,c,d)
      lua require('custom.statusbar.function_btn').showCwd()
   endfunction
]])
...
local iconCwd = function()
   ...
   local str = texthl(hl.cwdIcon, icon)
...
local widthCwdBelow = function(cwd)
   ...
   if not is_width then cwd = vim.fn.pathshorten(cwd) end

   return texthl(hl.text, cwd)
end
